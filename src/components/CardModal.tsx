...
import { LABELS, type Attachment, type Comment, type Member, type WorkSuspension, type Card } from '../types';
...
export default function CardModal({ cardId, boardId, onClose }: Props) {
  ...
  return <CardModalInner cardId={cardId} result={result} onClose={onClose} />;
}

type InnerProps = {
  cardId: string;
  result: { card: ReturnType<typeof useWorkStore.getState>['boards'][0]['columns'][0]['cards'][0]; column: ReturnType<typeof useWorkStore.getState>['boards'][0]['columns'][0] | null; board: ReturnType<typeof useWorkStore.getState>['boards'][0]; isInbox: boolean };
  onClose: () => void;
};

function CardModalInner({ cardId, result, onClose }: InnerProps) {
  const boards = useWorkStore(s => s.boards);
  ...
  const visibleBoards = useMemo(
    () => getVisibleBoards(currentUser?.email),
    [getVisibleBoards, currentUser?.email, boards]
  );
  const roadmaps = useMemo(() => visibleBoards.filter(b => b.type === 'roadmap'), [visibleBoards]);
  ...
  const [showAddWorkedRange, setShowAddWorkedRange] = useState(false);
  const [workedRangeFrom, setWorkedRangeFrom] = useState<string | null>(null);
  const [workedRangeTo, setWorkedRangeTo] = useState<string | null>(null);
  const [showWorkedDaysList, setShowWorkedDaysList] = useState(false);
      ...
      >
        ...
        <div className="modal-header" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
            ...
            <NeumorphicSelect
              ...
              options={[
                ...(isInbox ? [{ value: 'inbox', label: 'Inbox', icon: <Inbox size={12} /> }] : []),
                ...board.columns.map(c => ({
                  value: c.id,
                  label: c.name,
                })),
              ]}
              onChange={(targetColId) => {
        ...
        >
          ...
          <div className="card-modal-sidebar" style={{ display: 'flex', flexDirection: 'column', gap: 16, paddingLeft: 18, borderLeft: '1px solid hsl(var(--border) / 0.4)', minWidth: 0, width: '100%', boxSizing: 'border-box' }}>
            ...
            {/* Scheduled Start & Scheduled End (all boards) */}
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) minmax(0, 1fr)', gap: 10 }}>
                <div className="form-group" style={{ minWidth: 0, margin: 0 }}>
                  <label className="field-label" style={{ margin: '0 0 4px 0' }}>Scheduled Start</label>
                ...
                <div className="form-group" style={{ minWidth: 0, margin: 0 }}>
                  <label className="field-label" style={{ margin: '0 0 4px 0' }}>Scheduled End</label>
                  <NeumorphicDatePicker
                    value={card.dueDate}
                    onChange={newDue => {
              ...
              {/* Actual Start & Actual End — Roadmap boards only */}
              {isRoadmap && (
                <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) minmax(0, 1fr)', gap: 10 }}>
                  <div className="form-group" style={{ minWidth: 0, margin: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 4, marginBottom: 4, minWidth: 0 }}>
                      <label className="field-label" style={{ margin: 0, whiteSpace: 'nowrap' }}>Actual Start</label>
                      {card.actualStartDate && card.startDate && (() => {
                        ...
                        if (diff < 0) return (
                          <span style={{ fontSize: 9.5, fontWeight: 700, padding: '1px 5px', borderRadius: 6, backgroundColor: 'rgba(16,185,129,0.15)', color: '#10b981', flexShrink: 0, whiteSpace: 'nowrap' }}>
                            {Math.abs(diff)}d early
                          </span>
                        );
                        if (diff > 0) return (
                          <span style={{ fontSize: 9.5, fontWeight: 700, padding: '1px 5px', borderRadius: 6, backgroundColor: 'rgba(239,68,68,0.15)', color: '#ef4444', flexShrink: 0, whiteSpace: 'nowrap' }}>
                            {diff}d late
                          </span>
                        );
                        return null;
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 4, marginBottom: 4, minWidth: 0 }}>
                      ...
                      {(() => {
                        ...
                        if (actualOrProj) {
                          ...
                          return (
                            <span style={{ fontSize: 9.5, fontWeight: 700, padding: '1px 5px', borderRadius: 6, backgroundColor: 'rgba(59,130,246,0.15)', color: '#3b82f6', flexShrink: 0, whiteSpace: 'nowrap' }}>
                              On Target
                            </span>
                          );
                        } else {
                          ...
                          if (now.getTime() > targetDue.getTime() && !card.completed) {
                            ...
                            return (
                              <span style={{ fontSize: 9.5, fontWeight: 700, padding: '1px 5px', borderRadius: 6, backgroundColor: 'rgba(245,158,11,0.15)', color: '#f59e0b', flexShrink: 0, whiteSpace: 'nowrap' }}>
                                +{diffDays}d
                              </span>
                            );
                          }
            ...
            {/* Linked Project Roadmap (Standard boards only — allows linking a task to a Roadmap) */}
            {!isRoadmap && (
            ...
            {/* Work Suspensions (Paused Periods) — Roadmap boards only */}
            {isRoadmap && (
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      const updated = [...existing, { from: today, to: today, reason: '' }];
                      const cleanedDays = (card.workedDays || []).filter(d => !updated.some(s => d >= s.from.slice(0, 10) && d <= s.to.slice(0, 10)));
                      updateCard(cardId, {
                        workSuspensions: updated,
                        ...(cleanedDays.length !== (card.workedDays || []).length ? { workedDays: cleanedDays } : {})
                      });
                    }}
                  >
                    + Add Pause
                          ...
                          <div style={{ minWidth: 0 }}>
                            ...
                            <NeumorphicDatePicker
                              ...
                              onChange={val => {
                                ...
                                const cleanedDays = (card.workedDays || []).filter(d => !updated.some(sp => d >= sp.from.slice(0, 10) && d <= sp.to.slice(0, 10)));
                                updateCard(cardId, {
                                  workSuspensions: updated,
                                  ...(cleanedDays.length !== (card.workedDays || []).length ? { workedDays: cleanedDays } : {})
                                });
            ...
            {/* Daily Work Confirmation & Worked Days Tracking — Roadmap boards only */}
            {isRoadmap && (() => {
              const now = new Date();
              ...
              const loggedDates = Array.from(workStatus.workedDatesSet).sort();
              ...
              const handleSaveWorkedRange = () => {
                if (!workedRangeFrom) {
                  showToast('Please select a start date', 'warning');
                  return;
                }
                const fromDate = new Date(workedRangeFrom);
                const toDate = workedRangeTo ? new Date(workedRangeTo) : new Date(workedRangeFrom);
                ...
                const currentSet = new Set(card.workedDays || []);
                const cursor = new Date(fromDate);
                let addedCount = 0;
                while (cursor <= toDate) {
                  const dStr = toLocalDateString(cursor);
                  const isSuspended = (card.workSuspensions || []).some(s => {
                    const sFrom = s.from.slice(0, 10);
                    const sTo = s.to.slice(0, 10);
                    return dStr >= sFrom && dStr <= sTo;
                  });
                  if (!isSuspended && !currentSet.has(dStr)) {
                    currentSet.add(dStr);
                    addedCount++;
                  }
                ...
                const updates: Partial<Card> = { workedDays: sorted };

                // If actualStartDate is not set or later than fromDate, update it
                if (!card.actualStartDate || new Date(card.actualStartDate).getTime() > fromDate.getTime()) {
                  updates.actualStartDate = fromDate.toISOString();
                }

                updateCard(cardId, updates);
                showToast(`Logged ${addedCount} worked day(s)`, 'success');
                setShowAddWorkedRange(false);
                setWorkedRangeFrom(null);
                setWorkedRangeTo(null);
              };

              const handleFillFromStartToToday = () => {
                const today = new Date();
                today.setHours(0, 0, 0, 0);
                const cursor = new Date(actStart);
                cursor.setHours(0, 0, 0, 0);

                if (cursor.getTime() > today.getTime()) {
                  showToast('Start date is in the future', 'warning');
                  return;
                }
                ...
                updateCard(cardId, { workedDays: Array.from(currentSet).sort() });
                showToast(`Marked ${added} day(s) from start to today as worked`, 'success');
              };

              const handleRemoveWorkedDay = (dateStr: string) => {
                const updated = (card.workedDays || []).filter(d => d !== dateStr);
                updateCard(cardId, { workedDays: updated });
              };
              ...
              return (
                <div className="form-group" style={{ marginBottom: 16 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8, flexWrap: 'wrap', gap: 6 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      <button
                        ...
                        onClick={() => setShowAddWorkedRange(v => !v)}
                        title="Log a past worked date or date range"
                      >
                        <Plus size={11} /> Log Days
                      </button>
                      <button
                        type="button"
                      ...
                      >
                        {isWorkedToday ? 'Worked Today' : '+ Today'}
                      </button>
                    </div>
                  </div>

                  {/* 1. When Work Started (Editable DatePicker) */}
                  <div style={{
                    display: 'flex',
                    alignItems: 'center',
                    justifyContent: 'space-between',
                    ...
                    <div style={{ minWidth: 0 }}>
                      <div style={{ fontSize: 11, fontWeight: 600, color: 'hsl(var(--foreground))', display: 'flex', alignItems: 'center', gap: 5 }}>
                        <Clock size={12} color="#10b981" />
                        <span>Work Started</span>
                      </div>
                      <span style={{ fontSize: 9.5, color: 'hsl(var(--muted-foreground))' }}>
                        {card.actualStartDate
                          ? 'Confirmed actual start date'
                          : 'Set when work on this task began'}
                      </span>
                    </div>
                    <div style={{ width: 140, flexShrink: 0 }}>
                      <NeumorphicDatePicker
                        value={card.actualStartDate || null}
                        onChange={newActualStart => {
                          const patch: Partial<Card> = { actualStartDate: newActualStart };
                          if (newActualStart) {
                            const dStr = newActualStart.slice(0, 10);
                            const current = card.workedDays || [];
                            if (!current.includes(dStr)) {
                              patch.workedDays = [...current, dStr].sort();
                            }
                          }
                  ...
                  {/* 2. Add Worked Period Form (Expandable) */}
                  {showAddWorkedRange && (
                    <div style={{
                      padding: '10px',
                      borderRadius: 8,
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        ...
                        <button
                          type="button"
                          onClick={() => setShowAddWorkedRange(false)}
                          className="icon-btn"
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) minmax(0, 1fr)', gap: 6 }}>
                        <div>
                          <label style={{ fontSize: 9.5, fontWeight: 600, color: 'hsl(var(--muted-foreground))', display: 'block', marginBottom: 3 }}>From</label>
                          <NeumorphicDatePicker
                            value={workedRangeFrom}
                            onChange={setWorkedRangeFrom}
                            placeholder="Start date..."
                            align="left"
                          />
                        </div>
                        <div>
                          <label style={{ fontSize: 9.5, fontWeight: 600, color: 'hsl(var(--muted-foreground))', display: 'block', marginBottom: 3 }}>To</label>
                          <NeumorphicDatePicker
                            value={workedRangeTo}
                            onChange={setWorkedRangeTo}
                      ...
                      <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 6 }}>
                        ...
                        <button
                          ...
                          onClick={handleSaveWorkedRange}
                        >
                          <Check size={12} /> Save Days
                        </button>
                      </div>
                    </div>
                  )}

                  {/* 3. Status Notification Box */}
                  <div style={{
                    padding: '8px 10px',
                    borderRadius: 8,
                    ...
                    {/* Quick helper if start date is set and earlier than today with unworked gap */}
                    {card.actualStartDate && new Date(card.actualStartDate) < now && (
                      <div style={{ marginTop: 6, paddingTop: 6, borderTop: '1px dashed hsl(var(--border) / 0.6)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        <span style={{ fontSize: 10, color: 'hsl(var(--muted-foreground))' }}>
                          Started on {new Date(card.actualStartDate).toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}
                        </span>
                        <button
                          type="button"
                          onClick={handleFillFromStartToToday}
                          style={{
                            fontSize: 10,
                            fontWeight: 600,
                            color: '#10b981',
                        ...
                        >
                          Mark all days up to today as worked
                        </button>
                      </div>
                    )}
                  </div>

                  {/* 4. Logged Days View / Edit List */}
                  {loggedDates.length > 0 && (
                    <div style={{ marginTop: 6 }}>
                      <button
                        type="button"
                        onClick={() => setShowWorkedDaysList(v => !v)}
                        style={{
                          background: 'none',
                          border: 'none',
                          padding: 0,
                      ...
                      >
                        <span>{showWorkedDaysList ? 'Hide' : 'View'} logged work days ({loggedDates.length})</span>
                        <span style={{ fontSize: 9 }}>{showWorkedDaysList ? '▲' : '▼'}</span>
                      </button>
                      ...
                      {showWorkedDaysList && (
                        <div style={{
                          ...
                          {loggedDates.map(dStr => {
                            const isExplicit = (card.workedDays || []).includes(dStr);
                            ...
                            return (
                              <span
                                ...
                                style={{
                                  ...
                                  padding: '2px 6px',
                                  borderRadius: 5,
                                  backgroundColor: isExplicit ? 'rgba(16,185,129,0.12)' : 'hsl(var(--secondary) / 0.6)',
                                  color: isExplicit ? '#10b981' : 'hsl(var(--foreground))',
                                  border: `1px solid ${isExplicit ? 'rgba(16,185,129,0.3)' : 'hsl(var(--border) / 0.6)'}`,
                                  display: 'inline-flex',
                                  alignItems: 'center',
                                  gap: 4,
                                }}
                                title={isExplicit ? 'Explicitly logged day. Click × to unmark.' : 'Auto-registered by card activity.'}
                              >
                                {label}
                                {isExplicit && (
                                  <button
                                    type="button"
                                    onClick={() => handleRemoveWorkedDay(dStr)}
                                    style={{
                                      background: 'none',
                                      border: 'none',
                                      padding: 0,
            ...
            {/* Project Phase & Milestone — Roadmap boards only */}
            {isRoadmap && (
              <div className="form-group" style={{ marginBottom: 16 }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 6, marginBottom: 6 }}>
                  <label className="field-label" style={{ display: 'flex', alignItems: 'center', gap: 6, margin: 0, whiteSpace: 'nowrap' }}>
                  ...
                  >
                    <Milestone size={11} color={card.isMilestone ? '#fff' : 'hsl(var(--primary))'} />
                ...
                <NeumorphicSelect
                  ...
                  options={[
                    ...
                    ...(board?.sprints || []).map(sp => ({
                      value: sp.id,
                      label: `${sp.name} ${sp.status === 'active' ? '(Current Phase)' : `(${sp.status})`}`,
                    })),
                  ]}
                  onChange={val => {
                    updateCard(cardId, { sprintId: val || null });
                  }}
                  size="sm"
            ...
            {/* Automated Progress Indicator — Roadmap boards only */}
            {isRoadmap && (() => {
    ...
    {showCreateRoadmapModal && (
      <CreateRoadmapModal
        ...
        onClose={(createdRoadmapId?: string) => {
          ...
          if (createdRoadmapId) {
            updateCard(cardId, { linkedRoadmapId: createdRoadmapId });
            showToast(`Created & linked roadmap "${card.title}"`, 'success');
            onClose();
            switchBoard(createdRoadmapId);
            try {
              localStorage.setItem('worklane_current_page_v1', 'board');
              localStorage.setItem('worklane_current_board_id_v1', createdRoadmapId);
              localStorage.setItem('worklane_current_view_mode_v1', 'roadmap');
              window.dispatchEvent(new Event('storage'));
            } catch {}
          }
