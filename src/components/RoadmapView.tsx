...
import type { Board, Card as CardType, Member, Sprint, WorkSuspension } from '../types';
...
export function formatDateRange(start: Date | string | null | undefined, end: Date | string | null | undefined): string {
  ...
  const eStr = formatShortDate(end);
...
export function getCardWorkStatus(card: CardType, actualStartDate: Date, today: Date): CardWorkStatus {
  ...
  if (card.workSuspensions && card.workSuspensions.length > 0) {
    for (const s of card.workSuspensions) {
      ...
      while (cur <= toDate) {
        workedSet.delete(toLocalDateString(cur));
...
export default function RoadmapView({ board, onOpenCard, isObserver }: Props) {
  ...
  // View state: default to 'days' timeline
  ...
  // Draggable tasks table pane width with persistence
  const [tasksPaneWidth, setTasksPaneWidth] = useState<number>(() => {
    try {
      const saved = localStorage.getItem('worklane_roadmap_tasks_width');
      if (saved) {
        const parsed = parseInt(saved, 10);
        if (!isNaN(parsed) && parsed >= 340 && parsed <= 760) return parsed;
      }
    } catch {}
    return 540;
  });
  ...
  const handleMouseDownResize = (e: React.MouseEvent) => {
    ...
    const handleMouseMove = (moveEvent: MouseEvent) => {
      ...
      const newWidth = Math.max(340, Math.min(760, startWidth + delta));
      setTasksPaneWidth(newWidth);
    };

    const handleMouseUp = (upEvent: MouseEvent) => {
      const delta = upEvent.clientX - startX;
      const finalWidth = Math.max(340, Math.min(760, startWidth + delta));
      setTasksPaneWidth(finalWidth);
      try {
        localStorage.setItem('worklane_roadmap_tasks_width', String(finalWidth));
      } catch {}
      setIsResizing(false);
  ...
  // Measure canvas vertical scrollbar width so timeline header viewport precisely matches canvas viewport
  const [scrollbarWidth, setScrollbarWidth] = useState(0);

  useEffect(() => {
    const updateScrollbar = () => {
      if (timelineContainerRef.current) {
        const sbw = timelineContainerRef.current.offsetWidth - timelineContainerRef.current.clientWidth;
        setScrollbarWidth(Math.max(0, sbw));
      }
    };
    updateScrollbar();
  ...
  const handleTimelineWheel = (e: React.WheelEvent<HTMLDivElement>) => {
    ...
    const el = timelineContainerRef.current;
    if (el) {
      const isScrollableVertically = el.scrollHeight > el.clientHeight + 4;
      if (!isScrollableVertically || e.altKey) {
  ...
  // Keep timeline header synchronized horizontally when scale, zoom, or scrollbarWidth changes
  useEffect(() => {
    if (timelineContainerRef.current && timelineHeaderRef.current) {
      timelineHeaderRef.current.scrollLeft = timelineContainerRef.current.scrollLeft;
    }
  }, [currentDate, scale, zoomLevel, scrollbarWidth]);
  ...
  // Compute view date range (with past history buffer for left-scrolling)
  const { startDate: viewStart, endDate: viewEnd, intervals } = useMemo(() => {
    const base = new Date(currentDate);
    base.setHours(0, 0, 0, 0);

    if (scale === 'days') {
      // 14 days buffer before base date (for scrolling left) + 60 days forward
      ...
      // 4 weeks buffer before base date + 14 weeks forward
      const start = new Date(base);
      ...
      // 2 months buffer before base date + 8 months forward
      const start = new Date(base.getFullYear(), base.getMonth() - 2, 1);
      start.setHours(0, 0, 0, 0);
      const end = new Date(base.getFullYear(), base.getMonth() + 8, 1);
      end.setHours(0, 0, 0, 0);
  ...
  const allTasks = useMemo(() => {
    ...
    board.columns?.forEach((col, colIdx) => {
      ...
      col.cards?.forEach(card => {
        ...
        // Ensure actual track encompasses any logged work suspension dates
        if (card.workSuspensions && card.workSuspensions.length > 0) {
          for (const s of card.workSuspensions) {
            if (s.from) {
              const sStart = new Date(s.from);
              sStart.setHours(0, 0, 0, 0);
              if (!isNaN(sStart.getTime()) && sStart.getTime() < actStart.getTime()) {
                actStart = new Date(sStart);
              }
            }
            if (s.to) {
              const sEnd = new Date(s.to);
              if (s.to.length <= 10) sEnd.setHours(23, 59, 59, 999);
              if (!isNaN(sEnd.getTime()) && sEnd.getTime() > actOrProjEnd.getTime()) {
                actOrProjEnd = new Date(sEnd);
              }
            }
          }
        }
  ...
  const filteredTasks = useMemo(() => {
    return allTasks.filter(item => {
      ...
      // Status
      if (statusFilter === 'active' && (item.card.completed || item.card.actualEndDate)) return false;
      if (statusFilter === 'completed' && (!item.card.completed && !item.card.actualEndDate)) return false;
      if (statusFilter === 'overdue') {
        if (item.card.completed || item.card.actualEndDate || item.varianceDays < 0 || item.targetEndDate >= today) return false;
      }
  ...
  const groupedSections: GroupSection[] = useMemo(() => {
    ...
    if (groupBy === 'column') {
      const colPalette = ['#6366f1', '#3b82f6', '#10b981', '#f59e0b', '#8b5cf6', '#06b6d4', '#ec4899'];
      const columnSections: GroupSection[] = (board.columns || []).map((col, idx) => {
        const displayTitle = col.name === 'Project Deliverables' ? `Phase ${idx + 1}` : col.name;
        const matchingSprint = (board.sprints || []).find(s => s.id === col.id || s.name.toLowerCase() === col.name.toLowerCase());
        return {
          id: col.id,
          title: displayTitle,
          color: matchingSprint?.color || colPalette[idx % colPalette.length],
          ...
          tasks: scheduledTasks.filter(t => t.columnId === col.id || t.card.sprintId === col.id || (matchingSprint && t.card.sprintId === matchingSprint.id)),
        };
      });

      // Also include any sprints in board.sprints that didn't match an existing column
      (board.sprints || []).forEach((sp, sIdx) => {
        const alreadyCovered = columnSections.some(cs => cs.id === sp.id || cs.title.toLowerCase() === sp.name.toLowerCase());
        if (!alreadyCovered) {
          columnSections.push({
            id: sp.id,
            title: sp.name,
            subtitle: sp.goal || `${formatDueDate(sp.startDate)} – ${formatDueDate(sp.endDate)}`,
            color: sp.color || colPalette[(board.columns?.length || 0 + sIdx) % colPalette.length],
            badge: sp.status === 'active' ? 'CURRENT PHASE' : sp.status.toUpperCase(),
            sprint: sp,
            tasks: scheduledTasks.filter(t => t.card.sprintId === sp.id),
          });
        }
      });
  ...
  const completedCount = allTasks.filter(t => t.card.completed || !!t.card.actualEndDate).length;
  const overdueCount = allTasks.filter(t => !t.card.completed && !t.card.actualEndDate && t.targetEndDate < today && t.varianceDays >= 0).length;
  ...
  const handleJumpToday = () => {
    ...
    if (timelineContainerRef.current) {
      timelineContainerRef.current.scrollTo({ left: todayColumnLeftPx, behavior: 'smooth' });
    }
  };
  ...
  const handleSaveSprint = () => {
    ...
    if (editingSprint) {
      const sprintExists = (board.sprints || []).some(s => s.id === editingSprint.id);
      if (sprintExists) {
      ...
      const matchingCol = (board.columns || []).find(c => c.id === editingSprint.id || c.name.toLowerCase() === editingSprint.name.toLowerCase());
      if (matchingCol) {
        renameColumn(matchingCol.id, sprintName.trim());
      }
  ...
  const handleDeleteCard = (e: React.MouseEvent, cardId: string, cardTitle: string) => {
    e.stopPropagation();
    showConfirm({
      ...
      onConfirm: () => {
        deleteCard(cardId);
        showToast(`Deleted "${cardTitle}"`, 'info');
      }
    });
  };
  ...
  // Pixel sizing per interval with scale & zoom
  const baseIntervalWidth = scale === 'days' ? 52 : scale === 'weeks' ? 115 : 160;
  const intervalWidthPx = Math.round(baseIntervalWidth * zoomLevel);
  ...
  // Exact start X position of the Today column to ensure it is never cut off
  const todayColumnLeftPx = useMemo(() => {
    if (scale === 'days') {
      const idx = intervals.findIndex(d =>
        d.getFullYear() === today.getFullYear() &&
        d.getMonth() === today.getMonth() &&
        d.getDate() === today.getDate()
      );
      return idx >= 0 ? idx * intervalWidthPx : Math.max(0, todayXPos - intervalWidthPx / 2);
    } else if (scale === 'weeks') {
      const idx = intervals.findIndex(d => {
  ...
  }, [scale, intervals, today, intervalWidthPx, todayXPos]);

  // Auto-scroll timeline on mount and scale change to position Today's column at the leftmost visible edge cleanly
  useEffect(() => {
    const el = timelineContainerRef.current;
    if (!el) return;
    el.scrollLeft = todayColumnLeftPx;
    if (timelineHeaderRef.current) {
      timelineHeaderRef.current.scrollLeft = todayColumnLeftPx;
    }
  }, [board.id, scale, zoomLevel, todayColumnLeftPx]);
  ...
  const getStatusPill = (t: TaskWithSchedule) => {
    ...
    if (t.card.completed) {
      if (t.varianceDays < 0) {
        return { label: `Done (${Math.abs(t.varianceDays)}d early)`, color: '#10b981', bg: 'rgba(16,185,129,0.15)' };
      }
      if (t.varianceDays > 0) {
        return { label: `Done (${t.varianceDays}d late)`, color: '#f59e0b', bg: 'rgba(245,158,11,0.15)' };
      }
    ...
    // 2. Tasks with an explicit actual end date specified (finished early/late)
    if (t.card.actualEndDate) {
      if (t.varianceDays < 0) {
        return { label: `${Math.abs(t.varianceDays)}d early`, color: '#10b981', bg: 'rgba(16,185,129,0.15)' };
      }
      if (t.varianceDays > 0) {
        return { label: `${t.varianceDays}d late`, color: '#ef4444', bg: 'rgba(239,68,68,0.12)' };
      }
      return { label: 'On target', color: '#10b981', bg: 'rgba(16,185,129,0.12)' };
    }

    // 3. In-progress tasks ahead of scheduled deadline
    if (t.varianceDays < 0) {
      return { label: `${Math.abs(t.varianceDays)}d ahead`, color: '#10b981', bg: 'rgba(16,185,129,0.12)' };
    }

    // 4. Overdue tasks (scheduled deadline passed without completion/actual end date)
    if (t.targetEndDate < today) {
      const daysOverdue = Math.max(1, t.varianceDays > 0 ? t.varianceDays : Math.round((today.getTime() - t.targetEndDate.getTime()) / 86400000));
      return { label: `${daysOverdue}d overdue`, color: '#ef4444', bg: 'rgba(239,68,68,0.12)' };
    }

    // 5. Tasks projected behind schedule
    if (t.varianceDays > 0) {
      return { label: `${t.varianceDays}d behind`, color: '#f59e0b', bg: 'rgba(245,158,11,0.12)' };
    }
  ...
  const behindCount = allTasks.filter(t => !t.card.completed && !t.card.actualEndDate && t.varianceDays > 0).length;
    ...
    <div className={`roadmap-view-container roadmap-mobile-mode-${mobileTab}`}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                <h1 style={{ fontSize: 15, fontWeight: 700, margin: 0, color: 'hsl(var(--foreground))' }}>
                  {board.name}
                </h1>
                <span style={{
                  fontSize: 9.5,
              ...
              <p style={{ margin: '1px 0 0', fontSize: 11, color: 'hsl(var(--muted-foreground))' }}>
      ...
      <div style={{
        ...
        {[
          ...
          <div key={card.label} style={{
            ...
            <span style={{ fontSize: 18, fontWeight: 800, color: card.color, lineHeight: 1.1 }}>{card.value}</span>
            <span style={{ fontSize: 10.5, color: 'hsl(var(--muted-foreground))', fontWeight: 600 }}>{card.label}</span>
          </div>
        ))}
      ...
      <div style={{
        ...
        <div className="roadmap-scale-group" style={{ flexShrink: 0 }}>
          <button type="button" className={`scale-btn ${scale === 'days' ? 'active' : ''}`} onClick={() => { setScale('days'); setZoomLevel(1); }}>Days</button>
          <button type="button" className={`scale-btn ${scale === 'weeks' ? 'active' : ''}`} onClick={() => { setScale('weeks'); setZoomLevel(1); }}>Weeks</button>
          <button type="button" className={`scale-btn ${scale === 'months' ? 'active' : ''}`} onClick={() => { setScale('months'); setZoomLevel(1); }}>Months</button>
        </div>

        {/* Zoom Controls */}
        <div className="roadmap-zoom-group">
          <button
            type="button"
            className="roadmap-zoom-btn"
            onClick={() => setZoomLevel(z => Math.max(0.6, +(z - 0.2).toFixed(2)))}
            title="Zoom out timeline"
          >−</button>
          <button
            type="button"
            className={`roadmap-zoom-label ${zoomLevel !== 1 ? 'is-zoomed' : ''}`}
          ...
          <button
            type="button"
            className="roadmap-zoom-btn"
            onClick={() => setZoomLevel(z => Math.min(2.2, +(z + 0.2).toFixed(2)))}
            title="Zoom in timeline"
          >+</button>
        </div>

        {/* Date Navigation & Jump Today */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 4, flexShrink: 0 }}>
          <motion.button whileTap={{ scale: 0.92 }} type="button" className="icon-btn"
            onClick={() => handleNav('prev')} title="Go back" style={{ width: 24, height: 24, minWidth: 24 }}>
            <ChevronLeft size={13} />
          </motion.button>
          <span className="roadmap-date-label" style={{ fontSize: 11, fontWeight: 600, minWidth: 85, textAlign: 'center' }}>
            {viewStart.toLocaleDateString([], { month: 'short', year: 'numeric' })}
            {intervals.length > 0 && (viewStart.getMonth() !== intervals[intervals.length - 1].getMonth() || viewStart.getFullYear() !== intervals[intervals.length - 1].getFullYear()) && (
              <span> – {intervals[intervals.length - 1].toLocaleDateString([], { month: 'short', year: 'numeric' })}</span>
            )}
          </span>
          <motion.button whileTap={{ scale: 0.92 }} type="button" className="icon-btn"
            onClick={() => handleNav('next')} title="Go forward" style={{ width: 24, height: 24, minWidth: 24 }}>
            <ChevronRight size={13} />
          </motion.button>
          <motion.button whileTap={{ scale: 0.95 }} type="button"
            className="btn btn-secondary today-btn" onClick={handleJumpToday} title="Jump to today"
            style={{ fontSize: 10.5, padding: '1px 7px', height: 22 }}
          >Today</motion.button>
        </div>
        ...
        <div style={{
          ...
          <span style={{ display: 'flex', alignItems: 'center', gap: 3 }}>
            <span style={{ width: 8, height: 5, borderRadius: 2, background: '#10b981', display: 'inline-block' }} />
            <span style={{ fontWeight: 800, color: '#10b981', fontSize: 9 }}>[S]</span>
            Schedule
          </span>
          <span style={{ display: 'flex', alignItems: 'center', gap: 3 }}>
            <span style={{ width: 8, height: 5, borderRadius: 2, background: '#f97316', display: 'inline-block' }} />
            <span style={{ fontWeight: 800, color: '#f97316', fontSize: 9 }}>[A]</span>
            Actual
          </span>
          <span style={{ display: 'flex', alignItems: 'center', gap: 3 }}>
            <span style={{ width: 2, height: 10, borderRadius: 2, background: '#3b82f6', display: 'inline-block' }} />
            Today
          </span>
        </div>
      </div>
      ...
      <div className={`roadmap-split-pane mobile-tab-${mobileTab}`}>
        ...
        <div className="roadmap-tasks-pane" style={{ width: tasksPaneWidth }}>
          <div className="roadmap-pane-header">
            <div className="roadmap-col-header-cell task-col">Task</div>
            <div className="roadmap-col-header-cell sched-col" title="Baseline planned schedule (Start to Due Date)">
              <span className="roadmap-header-tag sched">S</span>
              <span>Schedule</span>
            </div>
            <div className="roadmap-col-header-cell act-col" title="Actual / Accomplished work period">
              <span className="roadmap-header-tag act">A</span>
              <span>Actual</span>
            </div>
            <div className="roadmap-col-header-cell status-col">Status</div>
          </div>
          ...
          <div className="roadmap-tasks-list" ref={taskListRef} onScroll={handleTaskListScroll}>
            {groupedSections.map(section => {
              ...
              return (
                <div key={section.id} className="roadmap-col-group">
                  ...
                  {!isCollapsed && (
                    <>
                      {/* Sub-header: Project Deliverables */}
                      <div className="roadmap-deliverables-header">
                        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                          <Layers size={12} color="hsl(var(--primary))" style={{ opacity: 0.8 }} />
                          <span style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.04em', color: 'hsl(var(--muted-foreground))' }}>
                            Project Deliverables
                          </span>
                          <span className="roadmap-deliverables-badge">{tasks.length}</span>
                        </div>
                      </div>

                      <div className="roadmap-col-rows">
                        {tasks.length === 0 ? (
                          <div className="roadmap-empty-col-row">No deliverables in this phase yet</div>
                        ) : (
                          tasks.map(t => {
                            ...
                            const plannedDays = Math.max(1, Math.round((t.targetEndDate.getTime() - t.targetStartDate.getTime()) / 86400000) + 1);
                            const hasStarted = Boolean(t.card.actualStartDate) ||
                              t.card.completed ||
                              (t.card.progress || 0) > 0 ||
                              t.isWorkedToday ||
                              t.workedDaysCount > 0 ||
                            ...
                            const actualTooltip = hasStarted
                              ? `Actual: ${formatShortDate(t.actualStartDate)} to ${formatShortDate(t.actualOrProjectedEndDate)}`
                              : `Not started (scheduled to begin ${formatShortDate(t.targetStartDate)})`;
                              ...
                              >
                                ...
                                <div className="roadmap-col-task">
                                  <button type="button" className="roadmap-check-btn"
                                    ...
                                    title={t.card.completed ? 'Mark incomplete' : 'Mark complete'}
                                  >
                                    {t.card.completed
                                  ...
                                  {!isObserver && (
                                    <button
                                      ...
                                      onClick={(e) => handleDeleteCard(e, t.card.id, t.card.title)}
                                      title={`Delete "${t.card.title}"`}
                                    >
                                      <Trash2 size={13} />
                                    </button>
                                  )}
                                  ...
                                  {isMilestone && (
                                    <div title="Key Milestone" className="roadmap-milestone-marker" />
                                  )}
                                ...
                                {/* 2. Schedule Column (S) */}
                                <div
                                  className="roadmap-col-sched"
                                  title={`Scheduled baseline plan: ${formatShortDate(t.targetStartDate)} to ${formatShortDate(t.targetEndDate)} (${plannedDays} days)`}
                                >
                                  <span className="roadmap-track-badge sched" title="Schedule / Planned (S)">S</span>
                                  <span className="roadmap-date-range">{formatDateRange(t.targetStartDate, t.targetEndDate)}</span>
                                  <span className="roadmap-duration-label">({plannedDays}d)</span>
                                </div>

                                {/* 3. Actual Column (A) */}
                                <div
                                  className="roadmap-col-act"
                                  title={actualTooltip}
                                >
                                  <span className="roadmap-track-badge act" title="Actual / Accomplished (A)">A</span>
                                  {hasStarted ? (
                                    <>
                                      <span className="roadmap-date-range">
                                        {formatDateRange(t.actualStartDate, t.actualOrProjectedEndDate)}
                                      </span>
                                      {t.startVarianceDays < 0 && (
                                        <span className="roadmap-variance-tag early" title={`Started ${Math.abs(t.startVarianceDays)} days early`}>
                                          {Math.abs(t.startVarianceDays)}d early
                                        </span>
                                      )}
                                      {t.varianceDays > 0 && !t.card.completed && (
                                        <span className="roadmap-variance-tag late" title={`${t.varianceDays} days behind schedule`}>
                                          +{t.varianceDays}d
                                        </span>
                                      )}
                                      {t.varianceDays < 0 && !t.card.completed && (
                                        <span className="roadmap-variance-tag early" title={`${Math.abs(t.varianceDays)} days ahead of schedule`}>
                                          {Math.abs(t.varianceDays)}d early
                                        </span>
                                      )}
                                    </>
                                  ) : (
                                    <span className="roadmap-date-range not-started">Not started</span>
                                  )}
                                </div>

                                {/* 4. Status Column (Desktop) */}
                                <div className="roadmap-col-status hide-on-mobile-flex">
                                  <span
                                  ...
                                  <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                                    {t.isWorkedToday && !t.card.completed && (
                                      <span className="roadmap-variance-tag active" title="Active today">
                                        Active
                                      </span>
                                    )}
                                    <div className="roadmap-row-progress-bar">
                                      <div
                                        className="roadmap-row-progress-fill"
                                        style={{
                                          width: `${progressPct}%`,
                                          background: t.card.completed ? '#10b981' : 'hsl(var(--primary))'
                                        }}
                                      />
                                    </div>
                                  </div>
                                ...
                                <div className="roadmap-col-mobile-meta show-on-mobile-flex">
                                  <span style={{ fontSize: 9, fontWeight: 700, padding: '1px 4px', borderRadius: 20, background: statusPill.bg, color: statusPill.color, whiteSpace: 'nowrap' }}>{statusPill.label}</span>
                                  <span style={{ fontSize: 9, color: 'hsl(var(--muted-foreground))' }}>{formatShortDate(t.targetEndDate)}</span>
                                </div>
                              </div>
                            );
                          })
                  ...
                  {/* ── Add Task footer row per section ── */}
                  {!isCollapsed && !isObserver && (
                    <div
                      className="roadmap-add-task-row"
                      onClick={() => {
                        setQuickTaskColumnId(section.id || section.tasks[0]?.columnId || board.columns?.[0]?.id || '');
                        setQuickTaskSprintId(section.sprint?.id || '');
                        setShowQuickAddModal(true);
                      }}
                      title="Add a new task to this group"
                    >
                      <Plus size={12} color="hsl(var(--muted-foreground))" />
                      <span>Add task</span>
                    </div>
                  )}
                </div>
            ...
            {groupedSections.length === 0 && (
              <div style={{ padding: '32px 16px 12px', textAlign: 'center' }}>
                ...
                <p style={{ margin: '4px 0 0', fontSize: 11.5, color: 'hsl(var(--muted-foreground))' }}>
                  Click below to add your first project phase.
                </p>
              </div>
            )}

            {/* ── Add Phase Row at bottom of list ── */}
            {!isObserver && (
              <div style={{ padding: '8px 12px 14px' }}>
                <button
                  ...
                  onClick={() => {
                    setEditingSprint(null);
                    setSprintName(`Phase ${(board.columns?.length || 0) + 1}`);
                    setSprintGoal('');
                    const now = new Date();
        ...
        {/* Resizer Divider between tasks table and timeline canvas */}
        <div
          className={`roadmap-pane-resizer ${isResizing ? 'is-resizing' : ''}`}
          onMouseDown={handleMouseDownResize}
          onDoubleClick={() => {
            setTasksPaneWidth(540);
            try { localStorage.setItem('worklane_roadmap_tasks_width', '540'); } catch {}
          }}
          title="Drag to resize columns · Double-click to reset"
        >
          <div className="roadmap-resizer-handle" />
        </div>

        {/* Right Side: Gantt Canvas */}
        <div className="roadmap-canvas-pane">

          {/* Timeline Column Headers (pinned at top, synced horizontally with timeline canvas) */}
          <div
            ref={timelineHeaderRef}
            onWheel={handleHeaderWheel}
            className="roadmap-timeline-header-track"
            style={{
              height: 42,
              overflow: 'hidden',
              flexShrink: 0,
          ...
          >
            <div className="roadmap-timeline-header" style={{ width: totalTimelineWidth, height: 42, borderBottom: 'none' }}>
          ...
          {/* Top-right corner filler matching the canvas vertical scrollbar */}
          {scrollbarWidth > 0 && (
            <div
              style={{
                position: 'absolute',
          ...
          <div
            ...
            className="roadmap-canvas-scroll-area"
            style={{
              flex: 1,
              minHeight: 0,
              overflow: 'auto',
          ...
          >
            ...
            {/* Bars area */}
            <div className="roadmap-bars-area" style={{ width: totalTimelineWidth, overflow: 'hidden' }}>
              {/* Grid background */}
              ...
              <div className="roadmap-bars-rows">
                {groupedSections.map(section => {
                  ...
                  let sprintBand = null;
                  if (section.sprint) {
                    const spStartDate = new Date(section.sprint.startDate);
                    spStartDate.setHours(0, 0, 0, 0);
                    const spEndDate = new Date(section.sprint.endDate);
                    spEndDate.setHours(23, 59, 59, 999);
                    const spStart = Math.max(viewStart.getTime(), spStartDate.getTime());
                    const spEnd = Math.min(viewEnd.getTime(), spEndDate.getTime() + 1);
                  ...
                  return (
                    <div key={section.id} className="roadmap-col-bars-group">
                      <div style={{ height: 30, position: 'relative', overflow: 'hidden' }}>
                        {sprintBand && (
                          <div className="roadmap-sprint-span-indicator" style={{
                            ...
                            <span style={{ fontSize: 9.5, fontWeight: 700, color: section.color || 'hsl(var(--primary))' }}>
                              {section.sprint?.name}
                            </span>
                          </div>
                        )}
                      ...
                      {!isCollapsed && (
                        <>
                          {/* Sub-header matching spacer */}
                          <div className="roadmap-deliverables-canvas-row" />
                          ...
                          {tasks.length === 0 ? (
                            ...
                            // Positions - snap end to next day midnight (+1 ms) so it cleanly fills the whole day column
                            const targetStartMs = Math.max(viewStart.getTime(), t.targetStartDate.getTime());
                            const targetEndMs = Math.min(viewEnd.getTime(), t.targetEndDate.getTime() + 1);
                            ...
                            const targetWidthPct = Math.max(0.5, targetRightPct - targetLeftPct);

                            const actualStartMs = Math.max(viewStart.getTime(), t.actualStartDate.getTime());
                            const actualEndMs = Math.min(viewEnd.getTime(), t.actualOrProjectedEndDate.getTime() + 1);
                            ...
                            const actualTotalWidthPct = Math.max(0.5, actualRightPct - actualLeftPct);

                            let fillCapMs: number;
                            if (t.card.completed) {
                              fillCapMs = actualEndMs;
                            } else if (t.isWorkedToday) {
                              // If work is confirmed today, fill through the entire end of today (full day)
                              const todayEndMs = new Date(today.getFullYear(), today.getMonth(), today.getDate() + 1, 0, 0, 0, 0).getTime();
                              fillCapMs = Math.min(actualEndMs, todayEndMs);
                            } else if (t.lastWorkedDate) {
                              // If last worked date, fill through the entire end of that worked day (full day)
                              const lastWorkedEnd = new Date(t.lastWorkedDate);
                              lastWorkedEnd.setHours(0, 0, 0, 0);
                              lastWorkedEnd.setDate(lastWorkedEnd.getDate() + 1);
                              fillCapMs = Math.min(actualEndMs, Math.max(actualStartMs, lastWorkedEnd.getTime()));
                            } else {
                            ...
                            const suspensionCount = (t.card.workSuspensions || []).length;
                            const tooltipText = [
                              t.card.title,
                              ...
                              t.card.completed
                                ...
                                : t.varianceDays > 0
                                  ...
                                  : t.varianceDays < 0
                                    ...
                                    : 'Status: On track',
                              t.startVarianceDays < 0 ? `Early Start: Started ${Math.abs(t.startVarianceDays)} day${Math.abs(t.startVarianceDays) === 1 ? '' : 's'} before scheduled date` : null,
                              suspensionCount > 0 ? `${suspensionCount} work pause${suspensionCount !== 1 ? 's' : ''} logged` : null,
                              !t.card.completed && (
                                t.isWorkedToday
                              ...
                              <div key={t.card.id} className="roadmap-bar-row">
                                ...
                                >
                                  <span style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', display: 'inline-flex', alignItems: 'center', gap: 4 }}>
                                    <span className="roadmap-bar-badge sched">S</span>
                                ...
                                >
                                  {t.startVarianceDays < 0 && (
                                    ...
                                    >
                                      <span className="roadmap-early-start-label">
                                        Early Start
                                      </span>
                                    </div>
                                  )}
                                  ...
                                  >
                                    {doneRatio > 0 && (
                                      <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}>
                                        <span className="roadmap-bar-badge act">A</span>
                                  ...
                                  {/* Work Suspension / Pause breaks cutting through the actual line */}
                                  {((t.card.workSuspensions || []) as WorkSuspension[]).map((s, sIdx) => {
                                    ...
                                    const sEndMs = sEnd.getTime() + 1;
                                    if (isNaN(sStartMs) || isNaN(sEndMs) || sEndMs <= sStartMs) return null;

                                    const clipStartMs = Math.max(actualStartMs, sStartMs);
                                    const clipEndMs = Math.min(actualEndMs, sEndMs);
                                    if (clipEndMs <= clipStartMs) return null;

                                    const trackDurationMs = Math.max(86400000, actualEndMs - actualStartMs);
                                    const suspLeftRelPct = Math.max(0, Math.min(100, ((clipStartMs - actualStartMs) / trackDurationMs) * 100));
                                    const suspRightRelPct = Math.max(0, Math.min(100, ((clipEndMs - actualStartMs) / trackDurationMs) * 100));
                                    const suspWidthRelPct = Math.max(1.0, suspRightRelPct - suspLeftRelPct);

                                    const suspTooltip = `Work Suspension / Pause\nFrom: ${formatShortDate(sStart)}\nTo: ${formatShortDate(sEnd)}${s.reason ? `\nReason: ${s.reason}` : ''}`;

                                    return (
                                      <div
                                        key={`susp-${sIdx}`}
                                        className="roadmap-suspension-gap"
                                        style={{
                                          left: `${suspLeftRelPct}%`,
                                          width: `${suspWidthRelPct}%`,
                                        }}
                                        title={suspTooltip}
                                        onClick={(e) => {
                                          e.stopPropagation();
                                          onOpenCard(t.card.id);
                                        }}
                                      >
                                        {suspWidthRelPct > 6 && (
                                          <span className="roadmap-suspension-badge">
                                            <span>⏸</span>
                                            <span>{s.reason || 'Paused'}</span>
                                          </span>
                                        )}
                                      </div>
                                    );
                      ...
                      {/* Spacer to match the left pane's "Add task" footer row */}
                      {!isCollapsed && !isObserver && (
                        <div style={{ height: 34 }} />
                      )}
                    </div>
                  ...
                  <div className="roadmap-modal-body">
                    <div className="form-group">
                      ...
                      <input
                        ...
                        placeholder="e.g. Design UI mockups, Implement user authentication..."
                        className="text-input"
                        autoFocus
                      />
                    </div>
                    ...
                    <div className="form-group">
                      <label className="field-label">Project Phase</label>
                      <NeumorphicSelect
                        ...
                        onChange={val => {
                          setQuickTaskColumnId(val);
                          const matchingSprint = (board.sprints || []).find(s => s.id === val || s.name.toLowerCase() === (board.columns || []).find(c => c.id === val)?.name.toLowerCase());
                          setQuickTaskSprintId(matchingSprint?.id || '');
                        }}
                        size="sm"
                      />
                    </div>
